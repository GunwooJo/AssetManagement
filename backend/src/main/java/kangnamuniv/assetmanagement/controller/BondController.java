...
public class BondController {
    ...
    @DeleteMapping("/bond/{id}")
    public ResponseEntity<ResponseDTO> deleteBond(@RequestHeader(value = "Authorization") String token,
        ...
        if (id <= 0) {
            return ResponseEntity.badRequest()
                    .body(new ResponseDTO(null, null, "id는 0보다 커야 합니다."));
        }

        try {
            bondService.deleteBondById(id);
            return ResponseEntity.ok(new ResponseDTO("삭제 성공", null, null));

        } catch (Exception e) {
            log.error("채권 삭제 중 에러 발생: ", e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new ResponseDTO(null, null, e.getMessage()));
        }
    }
